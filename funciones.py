...
def leer_csv(path:str, opcion:bool):
    ...
    with open(path, 'r', encoding="utf8") as archivo:   #encoding="utf8" fue la única solución que pude encontrar sobre el error de librerías que mi tiraba. Lo encontré en la página de https://stackoverflow.com
        ...
        if opcion:  #puse esto en caso de que otro .csv no lo necesite, pongo False en parametros
            vacio = lista.pop(-1)   #remuevo la última la lista ya que quedaba vacía y no me permitía seguir
            vacio = lista.pop(0)   
    return lista

def crear_lista_dict(path:str, key:str, key2:str, key3:str, key4:str, key5:str):
    lista_diccionario = list(map(lambda insumo: {key:insumo[0], key2:insumo[1], key3:insumo[2], key4:float(insumo[3]), key5:insumo[4]}, leer_csv(path, True)))
...
ID                Nombre                           Marca                  Precio                               Características
""")

    for item in lista:
        print(f"{item['ID']:^2s}    {item['nombre']:^32s}     {item['marca']:^25s}      ${item['precio']:.2f}     {item['caracteristicas']:<100s} ")
#--------------------------------------------------------------------------------------------------------------------------------------
...
def realizar_compras(lista:list, key:str, key2:str, key3:str, key4:str, key5:str):
...
def elegir_opcion(opcion):
    ...
    match opcion:
        case 1:
            mostrar_lista((crear_lista_dict(path, "ID", "nombre", "marca", "precio", "caracteristicas")))
        case 2:
